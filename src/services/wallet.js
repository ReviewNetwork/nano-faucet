...
const wallet = web3.eth.accounts.privateKeyToAccount(`0x${config.ETH_PRIVATE_KEY}`)
...
module.exports = {
  async sendEther ({ to, amount }) {
    ...
    let signedTransaction = await transaction.sign({ from: wallet.address, to, value, key: config.ETH_PRIVATE_KEY })
